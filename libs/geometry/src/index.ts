...
// Structural types inlined from the app (geometry is structurally compatible with
// the app's GeoJSON aliases — see the extraction plan). Kept internal to the lib.
type Feature = GeoJsonFeature<Geometry | null>;
type FeatureCollection = GeoJsonFeatureCollection<Geometry | null>;
type GeoJSON = Geometry | Feature | FeatureCollection;
type Pos2 = [number, number];
type BBox4 = [number, number, number, number];

interface IWrappedFeature<T = Feature> {
  id: number;
  at: string;
  ephemeral?: boolean;
 ...
 */
export function bufferPoint(point: {
  x: number;
  y: number;
...
export function findNearestPointOnLine(
  ...
  const result = turfNearestPointOnLine(
    line as LineString,
    point as Point,
    units ? { units: units as Units } : undefined,
  );
  return {
...
export function removeDegenerates(geometry: Geometry): Geometry | null {
  switch (geometry.type) {
    ...
    case "MultiPolygon": {
      ...
      if (components.every((c): c is Polygon => c.type === "Polygon")) {
        return {
          type: "MultiPolygon",
